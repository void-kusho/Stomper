//! Local JSON API and dashboard.
//!
//! Axum serves alert history from SQLite and traffic statistics from the shared in-memory
//! counters, plus the HTML/CSS/JS dashboard that renders them. It binds to loopback only:
//! there is no authentication, no TLS, and no remote access.
...
mod tests {
    ...
    async fn serves_stored_alerts_and_runtime_status() {
        ...
        assert!(page.contains("Network monitor"));

        let css = get(addr, "/dashboard.css").await;
        assert!(css.starts_with("HTTP/1.1 200 OK"));
        assert!(css.contains(":root"));

        let js = get(addr, "/dashboard.js").await;
        assert!(js.starts_with("HTTP/1.1 200 OK"));
        assert!(js.contains("renderAlerts"));
